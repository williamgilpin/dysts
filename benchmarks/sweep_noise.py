...
output_path = cwd + "/results/results_" + dataname + "_noise_sweep.json"
...
granularity_vals = np.linspace(0, 5, 10)
...
for equation_name in list(equation_data.dataset.keys()):
    ...
    for granularity in granularity_vals:
        ...
        train_data += granularity * np.std(train_data) * np.random.normal(size=train_data.shape[0])
